...
if [ -z "${PYTHON_BIN}" ]; then
    printf '%s\n' '{"bi_run_id":"","status":"ERROR","next_action":"REPORT_ERROR","reason_code":"PYTHON_RUNTIME_NOT_FOUND","artifact_refs":[],"message":"未找到可用 Python 运行时；请设置 YMB_WORKBUDDY_PYTHON","contract_version":1}'
    exit 1
fi
