...
v1.4.4 采用一个可安装 Skill、一个确定性 Coordinator 和两个按需 AI 角色：
...
- 若 WorkBuddy 提供图形界面：进入「设置 → 技能/Skills → 导入」，选择本目录或下方的
  `bank-statement-standardization_v1.4.4.zip` 单文件导入或解压安装。
...
本仓库可打包生成 `bank-statement-standardization_v1.4.4.zip`。三种装法：
```bash
# 通用：解压到目标客户端的 skills 目录（以 Kimi 为例）
unzip bank-statement-standardization_v1.4.4.zip -d ~/.kimi/skills/
```
- 支持「导入 zip」的客户端（如 WorkBuddy 图形界面）：直接在技能面板选择该文件导入。
- 重新打包（改动后）：运行 `python tools/release/package_skill.py`，从 `pyproject.toml` 读取版本并生成一个 `bank-statement-standardization_v1.4.4.zip`。
