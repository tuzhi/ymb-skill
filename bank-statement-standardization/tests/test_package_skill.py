...
class PackageSkillTest(unittest.TestCase):
    ...
    def test_package_excludes_runtime_raw_data_and_independent_tools(self):
        ...
        for path in (
            ...
            "runs/run-id/pipeline_result.json",
            "原始流水数据/source.xlsx",
    ...
    def test_packages_two_platform_harness_skills(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                ...
                [
                    "bank-statement-standardization_v1.4.12_macos.zip",
            ...
            for archive in archives:
                with zipfile.ZipFile(archive) as zf:
                    packages[archive.name] = {
                        ...
                        "pipeline_template": json.loads(zf.read(
                            "bank-statement-standardization/assets/pipeline_result.template.json"
                        )),
                    }

            macos = packages["bank-statement-standardization_v1.4.12_macos.zip"]
            windows = packages["bank-statement-standardization_v1.4.12_windows.zip"]
            ...
            for packaged in packages.values():
                ...
                self.assertEqual(
                    packaged["pipeline_template"]["skill_version"],
                    "1.4.12",
                )
                self.assertIn("bank-statement-standardization/roles/repair.md", names)
                self.assertIn("bank-statement-standardization/references/prompt-1-字段映射.md", names)
